...
STAGE=${EG_STAGE_ROOT:-$ROOT/envgate_tt}/$TASK
...
# tt variant: the task dir name is TaskTrove's path (r2egym-v1-NNNNN); the SIF is keyed by the Dockerfile hash,
# exactly as harbor's worker resolves it (glob "*-<sha256(Dockerfile)[:12]>.sif"), so the staged Dockerfile is required.
[[ -f "$STAGE/Dockerfile" ]] || { echo "{\"task\":\"$TASK\",\"mode\":\"$MODE\",\"error\":\"no_dockerfile\"}"; exit 0; }
DFH=$(sha256sum "$STAGE/Dockerfile" | cut -c1-12)
SIF=$(ls $ROOT/r2egym_sif/*-${DFH}.sif 2>/dev/null | head -1)
