...
RUN=${RUN:-snowball_ttband_p2o_distill_a}; CK=$E/$RUN/$RUN/checkpoints; CTL=snowball_ttband_ns_c_lr5e7_c
...
[ "$(cat $CK/latest_ckpt_global_step.txt 2>/dev/null)" = "12" ] && [ -f $CK/global_step_12/trainer_state.pt ] || { log "no complete checkpoint 12; no probes"; exit 1; }
# 1. mechanism gate from the gates log
python3 - $GATES <<'PY' || { log "MECHANISM GATE FAILED: shift_per_token did not fall by 30 % from steps 1-2 to 11-12; no probe spend"; exit 1; }
import re, sys, statistics
v = {}
for line in open(sys.argv[1]):
    m = re.search(r"^step (\d+) .*? shift=([-0-9.e+nan]+)", line)
    if m and m.group(2) not in ("nan", "-"): v[int(m.group(1))] = float(m.group(2))
early = [v[s] for s in (1, 2) if s in v]; late = [v[s] for s in (11, 12) if s in v]
...
# 2. export checkpoint 12
cd $C && X=$(RUN=$RUN STEP=12 sbatch --parsable --export=ALL,RUN=$RUN,STEP=12 export_hf.sbatch); echo "export $X $(date -Is)" >> $IDS; log "export job $X"
while squeue -h -j $X -o %T | grep -q .; do sleep 30; done
MD=$E/exports/${RUN}_step12/model; [ -f $MD/model.safetensors.index.json ] || { log "EXPORT MISSING ($MD)"; exit 1; }
MC=$E/exports/${CTL}_step12/model; [ -f $MC/model.safetensors.index.json ] || { log "control export missing ($MC)"; exit 1; }
# 3. probes
bash $E/p2o/build_s12_probe.sh p2od_s12_bare $MD > $E/p2o/build_p2od_s12_bare.log 2>&1 || { log "probe build failed (distill): $(tail -3 $E/p2o/build_p2od_s12_bare.log)"; exit 1; }
bash $E/p2o/build_s12_probe.sh p2oc_s12_bare $MC > $E/p2o/build_p2oc_s12_bare.log 2>&1 || { log "probe build failed (control): $(tail -3 $E/p2o/build_p2oc_s12_bare.log)"; exit 1; }
log "probes built"
...
cd $OTA; P1=$(DCFT=$PWD sbatch --parsable $E/p2od_s12_bare/sbatch/p2od_s12_bare_rl.sbatch); P2=$(DCFT=$PWD sbatch --parsable $E/p2oc_s12_bare/sbatch/p2oc_s12_bare_rl.sbatch)
[ -n "$P1" ] && [ -n "$P2" ] || { log "probe sbatch failed ($P1/$P2); cancelling fleet"; $W "scancel $F"; exit 3; }
echo "probe $P1 $(date -Is)" >> $IDS; echo "probe $P2 $(date -Is)" >> $IDS; log "probes $P1 (distill s12) $P2 (control s12)"
...
python3 $E/p2o/s12_compare.py $E/p2od_s12_bare/p2od_s12_bare/trace_jobs/eval_sessions/p2od_s12_bare_eval_step0 $E/p2oc_s12_bare/p2oc_s12_bare/trace_jobs/eval_sessions/p2oc_s12_bare_eval_step0 2>&1 | tee -a $LOG
log "DONE"
