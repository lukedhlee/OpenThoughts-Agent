...
E=/e/fscratch/reformo/lee27/experiments; J=$1; F=$2; DST=$3; STOP_STEP=${STOP_STEP:-12}
...
log "watch arm $J fleet $F run $DST stop at checkpoint $STOP_STEP"
while true; do
  ...
  if [ "$(cat $CK/latest_ckpt_global_step.txt 2>/dev/null)" = "$STOP_STEP" ] && [ -f $CK/global_step_$STOP_STEP/trainer_state.pt ] \
     && [ -z "$(find $CK/global_step_$STOP_STEP -mmin -2 2>/dev/null | head -1)" ] && squeue -h -j $J -o %T 2>/dev/null | grep -q .; then
    log "checkpoint $STOP_STEP complete -> scancel $J"; scancel $J
  fi
