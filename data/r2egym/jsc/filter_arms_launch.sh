...
declare -A EXPECT=([fmt]=81889 [beh]=83851)   # rows the survey census counted; filter_corpus.py exits non-zero on a mismatch
for rule in fmt beh; do
  ...
  OMP_NUM_THREADS=1 $PY $C/snowball/rst/filter_corpus.py --rule $rule --src $SRC --out $OUT --expect ${EXPECT[$rule]} >> $LOG 2>&1 || { say "$rule: filter_corpus.py failed (see $LOG)"; exit 1; }
