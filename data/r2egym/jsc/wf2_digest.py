...
CW, OW = 150, 110
ARMS = [("wf", "ttwf", T + "/r2egym-tt-wf300"), ("wf2", "ttwf2", T + "/r2egym-tt-wf2-112")]
...
def main():
    ...
    for task in tasks:
        ...
        if os.path.exists(ip): out.append("--- instruction.md (FIXED-clauses arm; the verbatim arm differs in the two test clauses and the assert line)"); out.append(textwrap.indent(open(ip, errors="replace").read()[:4500], "  "))
        summ = []
        for short, prefix, tree in ARMS:
            ...
            out.append("\n\n######## ARM %s (%s): %dW/%dL/%dN over %d attempts; showing %d" % (short, "TaskTrove-verbatim workflow" if short == "wf" else "FIXED-clauses workflow", counts["WIN"], counts["LOSS"], counts["NULL"], len(atts), len(chosen)))
