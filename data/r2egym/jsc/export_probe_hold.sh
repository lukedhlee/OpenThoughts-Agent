...
V=/e/fscratch/reformo/lee27/tasks/r2egym-raw-v3-val; CK=$E/$R/$R/checkpoints
...
python3 $C/make_snowball_probe.py --name $P --val-dir $V --k 8 --conc 256 --max-in 61440 --max-out 4096 --max-model-len 65536 \
  --nodes 12 --engines 8 --parity --eval-timeout 1800 --wall 05:00:00 --model $M > /dev/null 2>&1 || { echo "PROBE BUILD FAILED"; exit 1; }
...
cd $O && DCFT=$PWD sbatch --hold $E/$P/sbatch/${P}_rl.sbatch && echo "submitted ON HOLD (release with scontrol release)"
