...
CONC=${EG_CONC:-64}
# EG_SCRIPT selects the per-task gate script (envgate.sh = raw r2egym-NNNN tree, envgate_tt.sh = TaskTrove tree).
# EG_TASKLIST restricts the sweep to the tasks listed in that file (default: every dir under the staging root).
EG_SCRIPT=${EG_SCRIPT:-envgate.sh}
...
if [[ -n "${EG_TASKLIST:-}" ]]; then mapfile -t TASKS < <(grep -v "^#" "$EG_TASKLIST"); else mapfile -t TASKS < <(ls $STAGE_ROOT); fi
...
for T in "${TASKS[@]}"; do
  ...
  (
    for M in pristine oracle; do
      srun --jobid=$FLEET --overlap -N1 -n1 -w "$N" --time=${EG_SRUN_MIN:-40} \
        bash $R/${EG_SCRIPT:-envgate.sh} "$T" "$M" 2>/dev/null | grep "^{" >> $PARTS/$T.$M.json
    done
  ) &
