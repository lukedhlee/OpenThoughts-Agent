#!/usr/bin/env python3
"""gepa_worst.py <wave> <cand> [n] — the tasks a candidate should be reflected on.

Reads the wave's scores.csv and ranks the candidate's tasks by how badly it did against a reference arm:
...
from gepa_feat import AXES  # noqa: E402
...
ap.add_argument("--scores", default=None, help="default experiments/gepa/<wave>/scores.csv")
a = ap.parse_args()
sp = a.scores or "%s/gepa/%s/scores.csv" % (E, a.wave)
if not os.path.exists(sp):
    sys.exit("no scores at %s -- run gepa_score.py %s first" % (sp, a.wave))
rows = {}
...
print("# worst %d tasks for %s in wave %s (reference %s)\n" % (min(a.n, len(ranked)), a.cand, a.wave, a.ref))
