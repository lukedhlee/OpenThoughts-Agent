...
one() { r=$1; src=$E/$r; dst=$DST/$r
  ...
  # Verify = rsync dry-run itemize (per-file size+mtime; directory-time-only entries ignored) + regular-file count + the sum
  # of regular-file sizes. Not `du --apparent-size` of the tree: that includes directory st_size, which legitimately differs
  # between GPFS filesystems (lr3e7_kl01_a "failed" on an 8 KB directory-metadata delta, 2026-09-06).
  verify() { left=$(rsync -aHS --dry-run --itemize-changes "$src/" "$dst/" 2>/dev/null | grep -v -E '^\.d\.\.t' | head -3)
    n_src=$(find "$src" -type f | wc -l); n_dst=$(find "$dst" -type f | wc -l)
    b_src=$(find "$src" -type f -printf '%s\n' | awk '{s+=$1} END{print s+0}'); b_dst=$(find "$dst" -type f -printf '%s\n' | awk '{s+=$1} END{print s+0}')
    [ -z "$left" ] && [ "$n_src" = "$n_dst" ] && [ "$b_src" = "$b_dst" ]; }
  if [ -d "$dst" ] && verify; then echo "$(date '+%m-%d %H:%M') $r: existing copy verified, skipping cp" >> $LOG; else
    mkdir -p "$dst"
    cp -a --sparse=always "$src/." "$dst/" > "$dst.cp.log" 2>&1 || { echo "$(date '+%m-%d %H:%M') $r: CP FAILED (see $dst.cp.log)" >> $LOG; return; }
    verify || { echo "$(date '+%m-%d %H:%M') $r: VERIFY FAILED files $n_src/$n_dst bytes $b_src/$b_dst left='$left' - source kept" >> $LOG; return; }
  fi
