...
def make_blob(n_tokens, nonce=""):
    # code-like filler; measured 2.2 chars/token on the Snowball tokenizer (12000 req -> 16306 actual at 3.0)
    lines = []
    i = 0
    while sum(len(s) for s in lines) < n_tokens * 2.2:
        ...
        async with session.post(f"{a.url}/v1/chat/completions", json=body) as r:
            ...
            async for raw in r.content:
                ...
                if ch:
                    ...
                    if delta.get("content") is not None and (delta.get("content") != "" or ttft is None) and "role" not in delta:
                        nchunk += 1
                        if ttft is None:
...
async def run_sweep(session):
    ...
    for conc in [int(c) for c in a.conc.split(",")]:
        ...
        res["points"][conc] = s
        print(f"RESULT {a.label} sweep ctx={res['prefix_prompt_tokens']} conc={conc} agg_tps={f2(s['agg_tps'], 1)} "
              f"per_stream_tps={f2(s['per_stream_tps'], 2)} decode_tps_p50={f2(s['decode_tps_p50'], 2)} ttft_p50={f2(s['ttft_p50'], 2)} "
              f"ttft_p90={f2(s['ttft_p90'], 2)} wall={wall:.1f} errors={s['errors']}", flush=True)
        if s["errors"]:
...
async def run_prefill(session):
    ...
    for conc in [int(c) for c in a.conc.split(",")]:
        ...
        rs = await asyncio.gather(*[chat(session, m, 2) for m in msgs])
        ...
        res["points"][conc] = s
        print(f"RESULT {a.label} prefill conc={conc} prompt_tokens={s['prompt_tokens_p50']} prefill_tps={f2(s['prefill_tps'], 0)} "
              f"ttft_p50={f2(s['ttft_p50'], 2)} ttft_p90={f2(s['ttft_p90'], 2)} latency_p90={f2(s['latency_p90'], 2)} wall={wall:.1f} errors={s['errors']}", flush=True)
    return res
...
async def run_replay(session):
    ...
    print(f"RESULT {a.label} replay{'_forced' if a.forced else '_natural'} conc={conc} turns={s_st['n']} errors={s_st['errors']} "
          f"latency_p50={f2(s_st['latency_p50'], 1)} latency_p90={f2(s_st['latency_p90'], 1)} ttft_p50={f2(s_st['ttft_p50'], 2)} ttft_p90={f2(s_st['ttft_p90'], 2)} "
          f"decode_tps_p50={f2(s_st['decode_tps_p50'], 1)} completion_p50={s_st['completion_tokens_p50']} agg_tps={f2(s_st['agg_tps'], 0)} "
          f"turns_per_min={f2(s_st['turns_per_min'], 1)} mean_running={f2(s_st['mean_running'], 1)} prod_api_p50={f2(s_st['prod_api_p50'], 1)} "
          f"speedup_vs_prod_p50={f2(s_st['speedup_vs_prod_p50'], 2)} server_gen_tps={f2(res['server_gen_tps_overall'], 0)}", flush=True)
    return res
