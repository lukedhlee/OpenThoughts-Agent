...
S=$((SLURM_PROCID * 2))
"$V/bin/python" -u "$X/scale_rollout_probe.py" --tasks "$X/slices/slice_$S.json" --task-root "$T" --out "$RUN" --name c0 > "$RUN/c0.log" 2>&1 &
c0=$!
"$V/bin/python" -u "$X/scale_rollout_probe.py" --tasks "$X/slices/slice_$((S + 1)).json" --task-root "$T" --out "$RUN" --name c1 > "$RUN/c1.log" 2>&1 &
c1=$!
wait "$c0"; r0=$?
wait "$c1"; r1=$?
c0=''; c1=''
