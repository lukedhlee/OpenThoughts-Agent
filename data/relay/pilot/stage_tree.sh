#!/bin/bash
# stage_tree.sh [dest] — build the pilot's 100-task CalibForge tree on the Jupiter login node, from the public bundle.
#
...
[ "$n" = 100 ] || { echo "expected 100 task dirs in $DST, found $n"; exit 1; }
