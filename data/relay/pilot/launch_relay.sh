...
$PY $HERE/check_decide.py --check $RUNS/$CHECK --run3 $RUNS/relay_run3b_20260925 > $RUNS/$CHECK/check_decision.json
...
$PY $HERE/relay_plan.py --baseline $RUNS/$BASE --check $RUNS/$CHECK --check-decision $RUNS/$CHECK/check_decision.json \
  --out-tasks $RUNS/$NAME.solvable.txt > $RUNS/$NAME.plan.json
...
JOB=$(RELAY_PILOT_DIR=$HERE N_STUDENT=4 sbatch --parsable --export=ALL --nodes=8 --time=$TMIN --job-name=relay_full_relay $HERE/serve_relay.sbatch) \
  || { log "sbatch failed"; exit 1; }
log "relay serve job $JOB submitted (8 nodes, $TMIN min)"
tmux new -d -s relay_full_relay "ARMS=relay_repair NODES=8 CAP_NODE_H=$CEIL CONC=400 NTASKS=2043 TREE=$TREE TASK_LIST=$RUNS/$NAME.solvable.txt N_ATTEMPTS=$R OVF_AFTER=200 OVF_MAX=0.20 JOBS_ROOT=$JOBS_ROOT bash $HERE/run_pilot.sh $JOB $NAME"
