...
NAME=ifv2ho_${TAG}_$(date +%Y%m%d)${MODE:+_$MODE}; [ "$MODE" = full ] && NAME=ifv2ho_${TAG}_$(date +%Y%m%d)
...
TASKS=/e/data1/mmlaion/lee27/tasks/ifv2_heldout477 NTASKS=477 POLICY_FILE=ifv2_policy.yaml bash $W/run_tb2.sh $J $NAME $MODE 2>&1 | tee -a $LOG || { say "run_tb2 failed"; scancel $J; exit 1; }
RL=$E/logs/run_$NAME.log
