...
- 01:05 PT behavior_clip, measurement path: the dumped training batches (exports/dumped_data/*.jsonl) carry no log-prob fields (keys: prompt, response, reward, stop_reason, …), so the per-token rollout-vs-trainer distribution cannot be computed offline; the trainer must log it. MarinSkyRL branch lukedhlee/tis-ratio-tail-fractions (off lukedhlee/jupiter-parity64k = be413fcb, what the arms run): compute_tis_diagnostics now also emits tis/log_ratio_abs_gt_{0.05,0.1,0.2,0.3} and signed tis/log_ratio_gt_0.2 / lt_-0.2 (mask-weighted fractions, fixed keys; emitted for use_tis AND for rollout-logprob losses incl. behavior_clip). Tests: tests/cpu/utils/test_policy_optimization.py + test_tis_diagnostics_backends.py green locally (/tmp/tisdiag-venv). Not a PR (needs Luke's ask); ride it as --skyrl-ref / checkout on Jupiter's code/MarinSkyRL AFTER the two overfit runs finish (editable install shared with running jobs — do not switch it mid-run).
- A/B plan to confirm with Luke: baseline arm = current recipe (regular + TIS cap 2.0, eps .2/.2, staleness 2, lr 5e-7) launched on the new ref → its first 2–3 steps give the tail fractions; treatment = same config with policy_loss_type=behavior_clip, use_tis=false, eps_low=eps_high chosen so the clamp binds on ~1 % of tokens (the |log r| level where log_ratio_abs_gt_X ≈ .01, expected between .1 and .2). After the switch, ppo_clip_ratio_low/high report the binding fraction directly.

### 2026-09-07 01:10 PT — login02 outage; bridge and watchers moved to login01
- ~00:48 PT jpbl-s01-02 (login02) stopped accepting ssh (banner timeout) and bridge 9926 there stopped accepting connections (backlog full); the four GPU jobs pinned to http://10.128.1.2:9926 stalled (placeholder 1279/1280 done, head300 377/1280, sync overfit step 4, async step 3). Luke: "no login02, only jupiter01 and 03 for now". Cancelled 1701150 (complete anyway), 1701286, 1700921, 1701112. Every tmux on login02 (probe_watch_*, placeholder_readout, sub160_readout, hist_sequencer, apptainer_bridge_9926/9927, the JUWELS ControlMaster) is presumed dead.
- Replacement bridge started on login01 (jpbl-s01-01, 10.128.1.1:9926, tmux apptainer_bridge_9926, log apptainer_bridge/server_9926_login01.log). Pending Luke's TOTP: `ssh -M -S ~/.ssh/cm_juwels/bridge -N -f juwels` on login01, then `bash code/snowball/juwels_bridge2_setup.sh` (forward jwlogin03i:9926 → login01:9926); the live fleets (hist a/b/c until 03:42 PT, relief 14229714 12 h, placeholder 14229715 10 h) should reconnect. Relaunches must pin APPTAINER_BRIDGE_URL=http://10.128.1.1:9926 (probe_history.sh 5th arg; clone_sync sed line).
- Placeholder read-out (4 arms, sub160 split, min-scored 7 because one attempt never finished) running on login01: tmux placeholder_readout_l01 → hist_readouts/hist_placeholder_readout.md, log_login01. Overfit results so far (last:2, lr 1e-6): sync steps 1–4 reward .44/.39/.36/.44, entropy .43→.27, reasoning share .31→.38–.42, more length-stops; async steps 1–3 reward .42/.45/.45, entropy .43→.40, turns 37→26, declared-done .30→.33, reasoning .32→.46.

### 2026-09-07 17:10 UTC (10:10 PDT) — clock correction + relaunches on the login01 bridge
- CLOCK: every "PT" stamp in this file from ~"18:00 PT" 09-06 onward was the Mac's local clock, which had shifted to UTC+8 during the session; subtract 9 h for PDT (e.g. "00:48 PT" = 07:48 PDT = 14:48 UTC... no: Mac 00:48 = 16:48 UTC = 09:48 PDT). Cluster times (CEST = UTC+2) were right. From here: UTC + PDT.
- JUWELS master re-created on login01 by Luke (TOTP); the old login02 master still holds jwlogin03i:9926, so the login01 bridge is forwarded as jwlogin03i:9928 → login01:9926; new fleets 14231158-60 (3 × 32 nodes, 10 h, BRIDGE_PORT=9928). Relaunched: snowball_hist_head300_sub160b (overlay 014e7562, head:300, bridge http://10.128.1.1:9926) and snowball_async_last2_lr1e6_stale2b (1709557, staleness 2, last:2). Sync overfit not relaunched pending Luke. Placeholder read-out running on login01 (tmux placeholder_readout_l01).
- 17:35 UTC placeholder result (sub160, 157 tasks fully sampled at ≥7, paired vs keep on the same tasks): placeholder .314 (+.013 [-.017,+.043]) vs drop .327 (+.026) and last2 .324 (+.022) — all within noise of each other; ctx death .645 (keep .698, drop .624); turns 31; declared-done .344 (highest) but P(win|done) .716 (keep .80, drop .79); think share .95 overall, .98 at turn 20, .96 at 30, .85 at 40 (drop .43/.18/.07) at drop's prompt size (t20 27.9k vs keep 40.8k); completion tokens 880 (keep 951, drop 625); echo 3.1 % of recorded turns. Reading: placeholder delivers the window saving while the model keeps reasoning, but on 160 tasks the pass advantage over drop the report predicted (+1.9 pts) is not detectable (CI ±.03); it declares done more and is wrong more when it does. head:300 (relaunched as sub160b) is the remaining candidate; decision between keep / placeholder / head:300 for the hero run waits on it. Files: hist_readouts/hist_placeholder_{readout.md,curves.json}.

## 2026-09-07 10:40 PT (19:40 CEST) — fresh arm on the v2 split launched (Luke: "Yes fresh arm"), not a fulldist resume

`snowball_ttband_v2train_fulldist_gmm_seats1584_x16_a` job 1709720: Stage-3 base, `r2egym-tt-v2-train` ×16, fixed prompt + hardened
verifier, the other session's fast stack unchanged (staleness 2, grouped_mm, 1,584 seats, 12 coordinators, fulldist recipe KL .01 lr
5e-7 grad clip 1.0). Beat: resuming fulldist from step 42 (trained on all 993 → idval/oodval contaminated; only heldout clean).
Probes on val441 (idval + oodval + heldout) every 12 steps via `export_probe_v2.sh`. Sandboxes from JURECA (pool5 + v2a, 2,816
slots) because the JUWELS control master on Jupiter has expired (needs Luke's TOTP to re-add the 9922 forward).
- 19:45 UTC head:300 result (sub160b, 159 tasks at ≥7): mean .301 = keep .302; ctx death .703 (keep .698, drop .624, placeholder .645); turns 32; declared-done .294; P(win|done) .778; think share 1.00 at every depth (no echo, no drift) at drop's prompt size (t20 29.4k). Reading: cutting each earlier span to its first 300 characters keeps the model reasoning perfectly, and the window saving is real, but the model spends the extra turns and dies on context at keep's rate with keep's pass rate. Five contracts on the same 160 tasks: keep .302, drop .327, last2 .324, placeholder .314, head300 .301 — all within a ±.03 interval of each other. Files: hist_readouts/hist_sub160_five_{readout.md,curves.json}.
- 21:30 UTC overfit verdict (snowball_async_last2_lr1e6_stale2b, 10 steps, COMPLETED 4h20): under last:2 at lr 1e-6 the policy's reasoning share went .32 → .30 → .27 → .21 → .09 → .03 → .00 (steps 1–10), tokens/turn 1616 → 1002, declared-done .31 → .65, length-stops 154 → 23, entropy .44 → .13, reward flat (.48 → .47, range .38–.51). RL under a stripped contract removes thinking entirely within an epoch with no reward gain. DECISION INPUT: hero run under keep; drop/last:2/placeholder are out; head:300 keeps base thinking at 1.00 but the pressure acts on the completion, so it would likely erode too. The window has to be won by converging in fewer turns, not by giving more turns.
