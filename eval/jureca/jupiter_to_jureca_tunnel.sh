...
case "${1:-}" in
  ...
  up)
    ...
    # Bind only the login node's internal interface. Compute nodes can reach the
    # corresponding <login-host>i address, while the listener stays off the
    # public interface.
    REMOTE_BIND_HOST="${REMOTE_BIND_HOST:-$(
      ssh "${ssh_common[@]}" "$JURECA_USER@$JURECA_HOST" \
        'getent ahostsv4 "$(hostname -s)i" | awk "NR == 1 { print \$1; exit }"'
    )}"
    if [[ -z "$REMOTE_BIND_HOST" ]]; then
      echo "ERROR: could not resolve the JURECA login node's internal address." >&2
      exit 1
    fi

    echo "Attempting internal -R bind $REMOTE_BIND_HOST:$REMOTE_PORT ..."
    if ssh "${ssh_common[@]}" -f -N \
         -R "$REMOTE_BIND_HOST:$REMOTE_PORT:$VLLM_HOST:$VLLM_PORT" \
         "$JURECA_USER@$JURECA_HOST" 2>/dev/null; then
      echo "OK: internal bind succeeded."
      echo "Workers should use: http://<jureca-login-IB-host>:$REMOTE_PORT/v1"
      exit 0
    fi

    echo "Internal -R bind refused => falling back to loopback -R + an internal relay."
    ...
    ssh "${ssh_common[@]}" "$JURECA_USER@$JURECA_HOST" \
      "REMOTE_BIND_HOST=$REMOTE_BIND_HOST REMOTE_PORT=$REMOTE_PORT LOOPBACK_PORT=$LOOPBACK_PORT nohup setsid python3 -u -c '
import os, socket, threading, socketserver

HOST = os.environ[\"REMOTE_BIND_HOST\"]
