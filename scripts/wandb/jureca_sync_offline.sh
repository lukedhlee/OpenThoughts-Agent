...
JURECA_ENV="${JURECA_ENV:-$REPO_ROOT/hpc/dotenv/jureca.env}"

if [[ -f "$JURECA_ENV" ]]; then
  # shellcheck disable=SC1090
  source "$JURECA_ENV"
fi
OTAGENT_SECRETS="${OTAGENT_SECRETS:-${DC_AGENT_SECRET_ENV:-$HOME/.config/otagent/secrets.env}}"
...
wandb_root="${root_arg:-${WANDB_DIR:-${SCRATCH:-$HOME}/wandb}}"
