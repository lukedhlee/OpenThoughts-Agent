...
def validate_context_contract(config_path: Path) -> None:
    ...
    compaction = harbor.get("opencode_config", {}).get("compaction", {}).get("auto")
    ...
    if parsed.trainer.get("max_prompt_length") != EXPECTED_WINDOW - EXPECTED_OUTPUT:
        raise ValueError("trainer prompt allowance is inconsistent with the smoke window")
    generator = parsed.generator
    if generator.get("max_input_length") != EXPECTED_WINDOW - EXPECTED_OUTPUT:
        raise ValueError("generator prompt allowance is inconsistent with the smoke window")
